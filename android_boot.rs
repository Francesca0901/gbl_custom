...
use crate::utils::{
    aligned_subslice, find_gpt_devices, usize_add, usize_roundup,
...
fn avb_verify_slot<'a, 'b, 'c>(
    ...
    let preloaded = [("boot", kernel), ("vendor_boot", vendor_boot)];
    ...
    let res = slot_verify(
        ...
        &[cstr_literal!("boot"), cstr_literal!("vendor_boot")],
...
pub fn load_android_simple<'a>(
    ...
    let (ramdisk_load_buffer, _load) = load.split_at_mut(ramdisk_load_curr);
    
    // 移动内核尽可能向后以保留更多的空间。
    let kernel_tail_buffer_size = kernel_tail_buffer.len();
    let ramdisk_load_buffer_size = ramdisk_load_buffer.len();
    let (ramdisk, kernel) = images_buffer.split_at_mut(ramdisk_load_buffer_size);
...
pub fn android_boot_demo(entry: EfiEntry) -> Result<()> {
    ...
    let (ramdisk, _fdt, kernel, remains) = load_android_simple(&entry, &mut load_buffer[..])?;

    efi_println!(&entry, "");
    efi_println!(
        &entry,
        "Booting kernel @ {:#x}, ramdisk @ {:#x}",
    ...
    #[cfg(target_arch = "aarch64")]
    {
        ...
        unsafe { boot::aarch64::jump_linux_el2_or_lower(kernel, ramdisk, _fdt) };
    }
    ...
    #[cfg(target_arch = "riscv64")]
    {
        ...
        let _ = exit_boot_services(entry, remains)?;
        // SAFETY: We currently target at Cuttlefish emulator where images are provided valid.
        unsafe { boot::riscv64::jump_linux(kernel, boot_hart_id, _fdt) };
    }
}
