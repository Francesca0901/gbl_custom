...
pub fn get_efi_fdt<'a>(entry: &'a EfiEntry) -> Option<(&FdtHeader, &[u8])> {
    ...
    //         efi_println!(entry, "Found EFI Configuration Table Entry: {:?}", table.vendor_guid);
    //         if table.vendor_guid == EFI_DTB_TABLE_GUID {
    //             efi_println!(entry, "Found EFI_DTB_TABLE_GUID");
    //             // SAFETY: Buffer provided by EFI configuration table.
    //             return unsafe { FdtHeader::from_raw(table.vendor_table as *const _).ok() };
    //         }
    //     }
    //     efi_println!(entry, "EFI_DTB_TABLE_GUID not found in EFI Configuration Table");
    // } else {
    //     efi_println!(entry, "No EFI Configuration Table found");
    // }
    // None
